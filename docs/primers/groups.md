...
cleanup, not the primary validity mechanism. Any Admin can disband the
...
# Disband the group for everyone with a terminal withdrawal commit (any admin)
curl -X POST -H "Authorization: Bearer $TOKEN" \
...
the cache supersedes by revision and evicts on disband/withdrawal regardless of
TTL.
...
Important: admin-authored member add/remove and explicit disband now propagate
across subscribed peers, so removed members drop the space locally and disbanded
