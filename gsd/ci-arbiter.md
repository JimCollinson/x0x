...
Green of record for the ADR-0016 Phase 1 v0.26 re-home: draft mirror PR #6, <https://github.com/JimCollinson/x0x/pull/6>.

Historical pre-v0.26 fallback/rebase evidence lived on draft mirror PR #5, <https://github.com/JimCollinson/x0x/pull/5>.

- Repository: `JimCollinson/x0x`
- Branch: `feat/adr-0016-phase-1-on-v0.26` → fork `main`
- Status source: PR #6 Checks tab, per push

## Known-flake carve-out — daemon bring-up failures (internal arbiter only)

PR #6, and any successor fork-only mirror/branch CI for the ADR-0016 Phase 1 v0.26 re-home, counts as green of record when either:
...
2. the only red is one or more daemon bring-up/precondition failures, meaning all of:
   ...
   - **Isolation:** the number of timed-out tests is small — `<= 3` across all jobs — with the rest of the suite green. A larger or growing set of bring-up failures is treated as a possible real startup/networking regression and the mirror PR is red.
...
If any condition fails, the carve-out does not apply and the mirror PR is red under the normal blocking rule.
...
Scope: this governs the internal fork-only CI arbiter (currently PR #6) — the development gate — only. It is not a statement about David's upstream CI, and the flake remains flagged to David as a pre-existing harness issue.
