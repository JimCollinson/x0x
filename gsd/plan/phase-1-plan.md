...
7. **Independent review before the slice is accepted.** Each substantial slice gets a fresh **adversarial review** (independent reviewer, different model/provider where available) and a **craft review** (anchored to the repo's documented rules and sibling code — required because this is a contribution into a repo we don't maintain). Both run against the slice's diff at its checkpoint; verdicts are recorded in the checkpoint (or "Not run" with reason — never implied by omission). Blocking adversarial findings (CRITICAL/HIGH) and CONFORMANCE craft findings are fixed or explicitly accepted by Jim before the next slice is dispatched.
