...
Local implementation attempt `939ab8c fix(adr-0016-phase-1): retain withdrawn disband shell` is **not pushed and not accepted**. Local fmt/clippy/check and focused/parity tests pass, but independent code review found blockers: post-withdraw TreeKEM events can still be queued before terminality rejection; card-only withdrawn import can wipe a live group without roster/chain authority validation; and same-group alias records are not all marked withdrawn. These must be remediated before this PR note can describe Slice 5 as ready.
