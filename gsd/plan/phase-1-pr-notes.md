...
The earlier hidden/private propagation blocker is resolved by the `GroupDeleted` reparent in `a141d83`, but Slice 5 is still **blocked** at `f5cbe48` by a durable-terminality decision: after disband removes local `GroupInfo`, an old signed non-withdrawn card can recreate a local stub unless the daemon retains a persistent withdrawn tombstone / terminality memory keyed by stable group id. The approved dispatch had deferred “keep local withdrawn tombstone on every member”, so this now needs Jim/maintainer judgement before Slice 5 can be called done. Options: add durable tombstones now, explicitly risk-accept deferral, or retain a non-live withdrawn tombstone while hiding it from live-group APIs.
