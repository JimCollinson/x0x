...
## Provisional disband verb

The provisional user-facing verb is consistently `disband` in current CLI/API/docs/GUI surfaces. Earlier planning text described the maintainer-pending verb as a one-line swap; final Craft Review corrected this: because the verb is now deliberately present in CLI help, API descriptions, docs, and GUI copy, a maintainer-requested verb change before upstream PR will require a small multi-surface sweep, not literally one code-line. Do not overclaim the swap as one-line in the PR description.
